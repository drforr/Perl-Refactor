package Perl::Refactor::EnforcerConfig;
...
use Perl::Refactor::Utils qw< :booleans :characters severity_to_number >;
...
sub handle_extra_parameters {
    ...
    my $parameter_errors = $profile_strictness eq $PROFILE_STRICTNESS_WARN ?
        Perl::Refactor::Exception::AggregateConfiguration->new() : $errors;

    foreach my $offered_param ( $self->get_parameter_names() ) {
        $parameter_errors->add_exception(
            Perl::Refactor::Exception::Configuration::Option::Enforcer::ExtraParameter->new(
...
Perl::Refactor::EnforcerConfig - Configuration data for a Enforcer.
...
The value of C<set_themes> in the user's F<.perlrefactorrc>.


=item C< get_add_themes() >

The value of C<add_themes> in the user's F<.perlrefactorrc>.


=item C< get_severity() >

The value of C<severity> in the user's F<.perlrefactorrc>.
...
setting.  To be called by Perl::Refactor::Enforcer->new() once all valid
enforcers have been processed and removed from the configuration.
