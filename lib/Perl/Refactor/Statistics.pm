package Perl::Refactor::Statistics;
...
use Perl::Refactor::Utils::McCabe qw{ calculate_mccabe_of_sub };
...
sub accumulate {
    ...
    ## no refactor (RequireDotMatchAnything, RequireExtendedFormatting, RequireLineBoundaryMatching)
    ...
    $self->{_lines} += scalar @lines;
    {
        my ( $in_data, $in_pod );
        foreach ( @lines ) {
            if ( q{=} eq substr $_, 0, 1 ) {    ## no refactor (ProhibitCascadingIfElse)
...
Perl::Refactor::Statistics - Compile stats on Perl::Refactor violations.


=head1 DESCRIPTION

This class accumulates statistics on Perl::Refactor violations across one or
...
Create a new instance of Perl::Refactor::Statistics.  No arguments are supported
at this time.
...
The total number of subroutines analyzed by this Refactor.


=item C<statements()>

The total number of statements analyzed by this Refactor.


=item C<lines()>

The total number of lines of code analyzed by this Refactor.


=item C<lines_of_blank()>

The total number of blank lines analyzed by this Refactor. This includes only
blank lines in code, not POD or data.


=item C<lines_of_comment()>

The total number of comment lines analyzed by this Refactor. This includes only
lines whose first non-whitespace character is C<#>.


=item C<lines_of_data()>

The total number of lines of data section analyzed by this Refactor, not
...
The total number of lines of Perl code analyzed by this Refactor. Perl appearing
in the data section is not counted.


=item C<lines_of_pod()>

The total number of lines of POD analyzed by this Refactor. Pod occurring in a
data section is counted as POD, not as data.


=item C<violations_by_severity()>

The number of violations of each severity found by this Refactor as a
reference to a hash keyed by severity.


=item C<violations_by_enforcer()>

The number of violations of each enforcer found by this Refactor as a
reference to a hash keyed by full enforcer name.


=item C<total_violations()>

The total number of violations found by this Refactor.
