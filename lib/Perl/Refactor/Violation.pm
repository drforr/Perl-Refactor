package Perl::Refactor::Violation;
...
use Perl::Refactor::Utils qw< :characters :internal_lookup >;
...
sub new {
    ...
    # be creating new Perl::Refactor::Enforcer modules.

    if ( @_ != $CONSTRUCTOR_ARG_COUNT ) {
        throw_internal 'Wrong number of args to Violation->new()';
    }

    if ( eval { $elem->isa( 'Perl::Refactor::Document' ) } ) {
...
sub to_string {
    ...
    (my $short_enforcer = $long_enforcer) =~ s/ \A Perl::Refactor::Enforcer:: //xms;
...
Perl::Refactor::Violation - A violation of a Enforcer found in some source code.
...
=head1 SYNOPSIS
  ...
  my $vio  = Perl::Refactor::Violation->new($desc, $expl, $node, $sev);


=head1 DESCRIPTION

Perl::Refactor::Violation is the generic representation of an individual
Enforcer violation.  Its primary purpose is to provide an abstraction
layer so that clients of L<Perl::Refactor|Perl::Refactor> don't have to
know anything about L<PPI|PPI>.  The C<violations> method of all
L<Perl::Refactor::Enforcer|Perl::Refactor::Enforcer> subclasses must return a
list of these Perl::Refactor::Violation objects.
...
If you need to sort Violations by location, use this handy routine:
    ...
    @sorted = Perl::Refactor::Violation::sort_by_location(@violations);
...
Returns the name of the L<Perl::Refactor::Enforcer|Perl::Refactor::Enforcer>
...
Here are some examples:
    ...
    Perl::Refactor::Violation::set_format("%m at line %l, column %c.\n");
    # looks like "Mixed case variable name at line 6, column 23."

    Perl::Refactor::Violation::set_format("%m near '%r'\n");
    # looks like "Mixed case variable name near 'my $theGreatAnswer = 42;'"

    Perl::Refactor::Violation::set_format("%l:%c:%p\n");
    # looks like "6:23:NamingConventions::Capitalization"

    Perl::Refactor::Violation::set_format("%m at line %l. %e. \n%d\n");
