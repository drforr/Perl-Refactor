...
use Perl::Refactor::Exception::Parse ();
...
sub run {
    ...
    my ($violations, $had_error_in_file) = _refactor(\%options, @files);
...
sub _get_options {
    ...
    # @SEVERITY_NAMES variable is exported by Perl::Refactor::Utils.
...
sub _get_input {
    ...
    if ( !@args || (@args == 1 && $args[0] eq q{-}) )  {
        ...
        $code_string =~ m{ \S+ }xms || die qq{Nothing to refactor.\n};
        ...
        return map { -d $_ ? Perl::Refactor::Utils::all_perl_files($_) : $_ } @args;
    }
}
...
sub _refactor {
    ...
    my ( $opts_ref, @files_to_refactor ) = @_;
    @files_to_refactor || die "No perl files were found.\n";

    # Perl::Refactor has lots of dependencies, so loading is delayed
    ...
    for my $file (@files_to_refactor) {
        ...
        or do {
            if ( my $exception = Perl::Refactor::Exception::Parse->caught() ) {
                $had_error_in_file = 1;
...
sub _render_report {
    ...
    $verbosity =
        ($verbosity eq $DEFAULT_VERBOSITY && @files > 1)
            ...
            : $verbosity;
    my $fmt = Perl::Refactor::Utils::verbosity_to_format( $verbosity );
    if (not -f $file) { $fmt =~ s< \%[fF] ><STDIN>xms; } #HACK!
    Perl::Refactor::Violation::set_format( $fmt );
...
sub _render_theme_listing {
    ...
    require Perl::Refactor::ThemeListing;
    require Perl::Refactor;
...
sub _render_profile_prototype {
    ...
    require Perl::Refactor::ProfilePrototype;
    require Perl::Refactor;
...
sub _render_enforcer_docs {
    ...
    require Perl::Refactor;
    $refactor = Perl::Refactor->new(%opts);
    _set_up_pager($refactor->config()->pager());
...
Perl::Refactor::Command - Guts of L<perlrefactor|perlrefactor>.


=head1 SYNOPSIS

    use Perl::Refactor::Command qw< run >;
...
This is the implementation of the L<perlrefactor|perlrefactor> command.  You can use
...
Does the equivalent of the L<perlrefactor|perlrefactor> command.  Unfortunately, at
