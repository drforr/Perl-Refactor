package Perl::Critic::EnforcerFactory;
...
use Perl::Critic::Exception::Configuration::NonExistentEnforcer qw< >;
...
sub create_enforcer {
    ...
    if ( $args{-params} ) {
        ...
        $enforcer_config ||=
            Perl::Critic::EnforcerConfig->new( $enforcer_short_name );
    }
        ...
        if ( not exists $known_policies{$enforcer_name} ) {
            ...
            if ( $errors ) {
                $errors->add_exception(
                    Perl::Critic::Exception::Configuration::NonExistentEnforcer->new(
...
Perl::Critic::EnforcerFactory - Instantiates Enforcer objects.


=head1 DESCRIPTION

This is a helper class that instantiates
L<Perl::Critic::Enforcer|Perl::Critic::Enforcer> objects with the user's
...
Returns a reference to a new Perl::Critic::EnforcerFactory object.
...
B<-name> is the name of a L<Perl::Critic::Enforcer|Perl::Critic::Enforcer>
subclass module.  The C<'Perl::Critic::Enforcer'> portion of the name
...
L<Perl::Critic::Enforcer/"initialize_if_enabled"> invoked on it, so it
...
L<Perl::Critic::Enforcer|Perl::Critic::Enforcer> subclass that is
...
L<Perl::Critic::Enforcer/"initialize_if_enabled"> invoked on them, so
...
Perl::Critic::EnforcerFactory has a few static subroutines that are used
