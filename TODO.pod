...
Perl::Refactor::TODO - Things for Perl::Refactor developers to do

=head1 SEE ALSO

Perl-Refactor-More is a separate distribution for less-widely-accepted
enforcers.  It contains its own TODO.pod.
...
would work for Enforcers that dynamically determine the PBP references.
...
manually deal with Enforcers that behave oddly.
...
=item * Add queries to --list option to F<perlrefactor>.

List Enforcers based upon severity, theme, and (what I want this second)
applies_to.

=item * Add formatting of --list output.

Support Jeff Bisbee's use case (he dumps all the enforcers in severity order
...
test module that extracts the perl code from a template and refactor it.
...
ref). Not sure what enforcers are affected.
...
configuration block, analogous to the "-" sign used for disabling a enforcer,
e.g. "C<[+Example::Enforcer]>".


=item * Threading

Pretty obviously, Perl::Refactor is readily parallelizable, just do a document per
...
=item * Add support for "## use refactor (blah)".

If I've got:

    ## no refactor (SomeEnforcer)

    ...

    ## no refactor (ADifferentEnforcer)

    ...

    ## no refactor (YetAnotherEnforcer)

If I want to turn C<YetAnotherEnforcer> back on but neither C<SomeEnforcer> nor
C<ADifferentEnforcer>, I've got to do this:

    ## use refactor
    ## no refactor (SomeEnforcer, ADifferentEnforcer)

Why can't I do this:

    ## use refactor (SomeOtherEnforcer)
...
This is necessary to get P::C::Moose Enforcers correct.
...
at $work at that time (and I really wanted to use Perl::Refactor at work).
...
Document bugs for individual Enforcers in the Enforcers themselves.  Users
...
The idea behind this enforcer is to encourage better names for variables
...
them up in a dictionary (see PodSpelling).  This enforcer should probably have
...
Forbid C<do "foo.pl">.  Not sure about this enforcer name.
...
applying this enforcer.  Low severity.
...
does the new enforcer buy us? Could we get the same thing with an option on the
...
=item * Give L<Perl::Refactor::Command> a proper API.

Now that we've got the guts of L<perlrefactor> in there, we should make the
it available to users.


=item * Create constants for the PPI location array elements.

=item * Some means of detecting "runaway" C<##no refactor>

Elliot was talking to a couple of users at ETech and one of their major
concerns was that they were using C<##no refactor> and forgetting to do a
C<##use refactor> after the problematic section.  Perhaps an option to
F<perlrefactor> to scan for such things is in order.
...
=item * Enhance P::C::refactor() to accept files, directories, or code strings

Just like F<bin/perlrefactor> does now.


=item * Add C<-cache> flag to F<bin/perlrefactor>

If enabled, this turns on L<PPI::Cache|PPI::Cache>:

    require PPI::Cache;
    my $cache_path = "/tmp/test-perl-refactor-cache-$ENV{USER}";
...
In several places, Perl::Refactor uses C<List::MoreUtils::any> to see if a
...
wrote Perl::Refactor:Utils::PPIRegexp to encapsulate this messiness, but it
